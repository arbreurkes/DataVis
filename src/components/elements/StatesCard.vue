      ...
      d3.json("/data/uscounties.json").then(json => {
        ...
        d3.csv("/data/stateData.csv").then(data => {
          d3.csv("/data/countyData.csv").then(countyData => {
            ...
            var countyStatOne = countyData.map((x) => parseFloat(x[that.statOne])).sort(sortNumeric);
            var countyStatTwo = countyData.map((x) => parseFloat(x[that.statTwo])).sort(sortNumeric);
